...
public class CommandManager implements Listenable {
    ...
    @EventHandler
    private Listener<ChatEvent> chatEventListener = new Listener<>(event -> {
        if (event.message.startsWith(".")) {
            event.cancel();
            String command = event.message.substring(1).split(" ")[0];
            List<String> _args = new ArrayList<String>(Arrays.asList(event.message.split(" ")));
    ...
    public static ArrayList<Class<? extends Command>> findCommands() {
        ...
        ArrayList<Class<? extends Command>> list = new ArrayList<Class<? extends Command>>();
        list.addAll(reflections.getSubTypesOf(Command.class));
        return list;
    }
}
